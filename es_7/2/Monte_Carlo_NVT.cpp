...
int main()
{ 
remove("output.epot.0"); 
remove("output.gave.0");
remove("output.gofr.0"); 
remove("output.pres.0"); 
...
double epot_eq[n_blocchi];
double pres_eq[n_blocchi];
...
//g(r)
for (int i=0; i<100; i++){
	double deltav=(4/3)*M_PI*(pow((0.005+0.01*i+0.01)*box,3)-pow((0.005+0.01*i)*box,3));
    	gr[i]=blk_av[i+2]/blk_norm/rho/m_part/deltav;
    	//glob_av[i+2] += gr[i];
	//glob_av2[i+2] += gr[i]*gr[i];
	//gr_ave[i]=glob_av[]/(double)iblk;
	//gr_err[i]=Error(glob_av[i+2],glob_av2[i+2],iblk);
    	Gofr<<gr[i]<<endl;
    }
